...
if options[:version]
  ...
  successful_build = system("docker build -t #{registry}/#{image_name}:latest -t #{registry}/#{image_name}:#{options[:version]} .")
...
if options[:push]
  ...
  successful_push = system("docker push #{registry}/#{image_name}:#{current_version}") &&
