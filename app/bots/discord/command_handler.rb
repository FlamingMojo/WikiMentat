module Discord
  module CommandHandler
    ...
    def handle_command(command, service, defer: true)
      Discord.bot.application_command(command) do |event|
        EventHandler.new(event: event, service: const_get(service), defer: defer).respond
      end
    end

    def handle_button(custom_id, service, defer: true)
      Discord.bot.button(custom_id: custom_id) do |event|
        EventHandler.new(event: event, service: const_get(service), defer: defer).respond
      end
    end

    def handle_select_menu(custom_id, service, defer: true)
      Discord.bot.select_menu(custom_id: custom_id) do |event|
        EventHandler.new(event: event, service: const_get(service), defer: defer).respond
      end
    end

    def handle_user_select(custom_id, service, defer: true)
      Discord.bot.user_select(custom_id: custom_id) do |event|
        EventHandler.new(event: event, service: const_get(service), defer: defer).respond
      end
    end

    def handle_role_select(custom_id, service, defer: true)
      Discord.bot.role_select(custom_id: custom_id) do |event|
        Discord::EventHandler.new(event: event, service: const_get(service), defer: defer).respond
      end
    end

    def handle_modal(custom_id, service, defer: true)
      Discord.bot.modal_submit(custom_id: custom_id) do |event|
        EventHandler.new(event: event, service: const_get(service), defer: defer).respond
      end
    end
    ...
    def handle_group(command, group_name, subcommands, defer: true)
      Discord.bot.application_command(command).group(group_name) do |group|
    ...
    def handle_reaction(service, **attributes)
      Discord.bot.reaction_add(attributes) do |event|
        const_get(service).new(event).handle
      end
    end

    def handle_message(service, **attributes)
      Discord.bot.message(attributes) do |event|
        const_get(service).new(event).handle
      end
    end
    ...
    class EventHandler
      ...
      def respond
        ...
        event.respond(
          content: DiscordError.handle(error:, user: User.new(discord_uid: event.user.id), service: service.to_s),
          ephemeral: true,
        )
      end
